...
class ChatConsumer(AsyncJsonWebsocketConsumer):
	...
	async def receive_json(self, content):
		...
		try:
			if command == "join":
				...
				if payload != None: # Decode JSON file and send through send_messages_payload function
					payload = json.loads(payload)
					await self.send_messages_payload(payload['messages'], payload['new_page_number'])
				else:
					raise ClientError(204, "Something went wrong retrieving the chat messages.")
	...
	async def send_messages_payload(self, messages, new_page_number):
		...
		await self.send_json({ # Not supposed to broadcast to entire group; only to user, which is what this is doing
...
@database_sync_to_async
def get_room_chat_messages(room, page_number): # Get chat room messages or None
	try:
		...
		if new_page_number <= p.num_pages: # More pages to go through
